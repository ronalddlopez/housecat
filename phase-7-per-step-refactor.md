...
| `backend/main.py` | Fix QStash `receiver.verify()` URL (line 86 only — callers are already correct) |
...
class TestStep(BaseModel):
    ...
    tinyfish_goal: str = Field(default="", description="The TinyFish goal prompt for just this step, including JSON output format")
```

**Why `default=""`:** The planner AI agent (Claude Haiku) generates `TestStep` objects. If Haiku omits `tinyfish_goal` from any step, a required field would cause a hard Pydantic `ValidationError` that crashes the entire pipeline. Using `default=""` prevents the crash; the `create_plan()` validation below catches empty values and retries gracefully.
...
**Update `create_plan()` to validate per-step goals and retry if missing:**
...
async def create_plan(url: str, goal: str) -> TestPlan:
    for attempt in range(3):
        ...
        # Validate every step has a non-empty tinyfish_goal
        missing = [s.step_number for s in plan.steps if not s.tinyfish_goal or len(s.tinyfish_goal.strip()) < 10]
        if not missing:
            return plan

        print(f"[Planner] Attempt {attempt + 1}: steps {missing} missing tinyfish_goal, retrying...")

    # Final attempt failed — use the combined goal as fallback for any missing steps
    for step in plan.steps:
        if not step.tinyfish_goal or len(step.tinyfish_goal.strip()) < 10:
            step.tinyfish_goal = f"{step.description}. Navigate to {url}, perform this action, and verify the result. Return JSON: {{\"success\": true/false, \"action_performed\": \"...\", \"verification\": \"...\", \"error\": null}}. Return valid JSON only."
    return plan
```

This guards against the biggest risk in the refactor: the planner AI omitting per-step `tinyfish_goal` values. It retries up to 3 times, and on final failure, auto-generates a reasonable fallback from the step description.
...
**Note about `step_executions` field:** We pass the `StepExecution` objects directly to `BrowserResult` (not `.model_dump()` dicts). Pydantic handles serialization when `result_store` calls `browser_result.model_dump()` later. This avoids unnecessary double-serialization.
...
def store_run_result(test_id: str, final_result, plan, browser_result, triggered_by: str = "manual", screenshots: list | None = None) -> dict:
    ...
    # Extract step_executions from browser_result (these are StepExecution objects)
    step_executions_raw = []
    if hasattr(browser_result, 'step_executions') and browser_result.step_executions:
        step_executions_raw = [
            se.model_dump() if hasattr(se, 'model_dump') else se
            for se in browser_result.step_executions
        ]
    ...
    run_record = {
        ...
        "step_executions": step_executions_raw,
...
## Fix 8: Frontend — Test Detail Page (`client/src/pages/test-detail.tsx`)
...
## Fix 9: Browser Preview Iframe (`client/src/components/live-execution-panel.tsx`)
...
| # | File | Change |
|---|------|--------|
| 1 | `backend/models.py` | Add `StepExecution` model, add `tinyfish_goal` (default `""`) to `TestStep`, add `step_executions` (default `[]`) to `BrowserResult` |
| 2 | `backend/agents/planner.py` | Update instructions for per-step goals + add validation/retry in `create_plan()` |
| 3 | `backend/agents/browser.py` | Add `execute_step()` function for single-step TinyFish calls |
| 4 | `backend/agents/pipeline.py` | Loop over steps individually, capture screenshot per step, pass `StepExecution` objects directly |
| 5 | `backend/services/screenshot.py` | Replace `capture_step_screenshots` with `capture_before_after` |
| 6 | `backend/services/result_store.py` | Store `step_executions[]` in run record (serialized via `.model_dump()`) |
| 7 | `backend/main.py` | Fix QStash `receiver.verify()` URL (line 86 only) |
| 8 | `client/src/pages/test-detail.tsx` | Add `StepExecution` interface, per-step screenshots/evidence/raw-json with fallbacks |
| 9 | `client/src/components/live-execution-panel.tsx` | Replace iframe with "Open Live Preview" button |

**No changes needed:** `backend/api/tests.py`, `backend/api/results.py`, `backend/run_pipeline.py`, `backend/services/tinyfish.py`, `backend/agents/evaluator.py`, `client/src/pages/run-test.tsx` (all use preserved fields/signatures)

## Breaking Change Prevention

These safeguards were added after a comprehensive investigation of all callers and consumers:

### 1. `TestStep.tinyfish_goal` defaults to `""` (not required)

The planner AI agent (Claude Haiku) generates `TestStep` objects via structured output. If the agent omits `tinyfish_goal` from any step, a **required** field would hard-crash with `ValidationError`. Using `default=""` prevents the crash. The `create_plan()` function validates and retries up to 3 times, with a fallback that auto-generates goals from step descriptions.

### 2. `BrowserResult.step_executions` defaults to `[]`

Uses `default_factory=list` so all existing code that constructs `BrowserResult` without `step_executions` still works. The browser agent (which outputs `BrowserResult` as structured AI output) won't break either — the field simply defaults to empty.

### 3. `StepExecution` objects passed directly (not `.model_dump()` dicts)

Pipeline passes `StepExecution` objects directly to `BrowserResult`. The `result_store` calls `.model_dump()` when serializing to Redis. This avoids double-serialization and keeps types correct throughout the chain.

### 4. Frontend fallback for old runs

The Evidence tab, Raw JSON tab, and Screenshots tab all check for `step_executions` first, then fall back to the legacy `tinyfish_data` / `tinyfish_raw` fields. Old runs stored in Redis before this refactor will render correctly.

### 5. `run-test.tsx` is unaffected but not updated

`client/src/pages/run-test.tsx` (the ad-hoc "Run Test" page) consumes `result.browser_result.streaming_url`, `result.result.step_results`, and `result.plan.steps` — all of which are preserved in this refactor. It won't display the new per-step execution data, but it won't break either. Updating it to show per-step data is optional and can be done later.

### 6. All 4 callers of `run_test()` are safe

The return signature stays `tuple[TestPlan, BrowserResult, TestResult, list]`. These callers all unpack the same 4-tuple and need no changes:
- `backend/main.py` (QStash callback, line 100)
- `backend/main.py` (manual run, line 164)
- `backend/api/tests.py` (run endpoint, line 72)
- `backend/run_pipeline.py` (CLI, line 16)
